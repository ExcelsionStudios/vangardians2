...
public class FlingProjectile : MonoBehaviour {
	...
	private float timeUntilPrediction = 0.5f;
	...
	void Update () {
		...
		{
			timeUntilPrediction -= Time.deltaTime;
			if (timeUntilPrediction <= 0f)
			{
				timeUntilPrediction = predictionInterval;
				...
				GameObject newProjectile = (GameObject)Instantiate(predictionPrefab, gameObject.transform.position, predictionPrefab.transform.rotation);
